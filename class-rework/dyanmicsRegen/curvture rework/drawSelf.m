function fig = drawSelf(y,f,t)
    ...
    stanceLegX = footX + (d.*cos(tht0 + q0.*s))/10 + (cos(tht0 + q0.*s) - cos(tht0))./q0;
    stanceLegY = footY + (d.*sin(tht0 + q0.*s))/10 + (sin(tht0 + q0.*s) - sin(tht0))./q0;

    swingLegX = stanceLegX + (d.*cos(tht1 + q1.*s))./10 + (d.*cos(q0 + tht0))./10 + (cos(q0 + tht0) - cos(tht0))./q0 ...
        + (sin(tht1) - sin(tht1 + q1.*s) + q1.*(cos(tht1) - cos(tht1 + q1.*s) + s.*cos(tht1 + q1.*s)))./q1.^2;
    swingLegY = stanceLegY + (d.*sin(tht1 + q1.*s))./10 + (d.*sin(q0 + tht0))./10 + (sin(q0 + tht0) - sin(tht0))./q0 ...
        + (cos(tht1 + q1.*s) - cos(tht1) + q1.*(sin(tht1) - sin(tht1 + q1.*s) + s.*sin(tht1 + q1.*s)))./q1.^2;
