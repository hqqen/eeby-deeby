close all; clear all;
syms q0 dq0 q1 dq1 tht0 tht0des dtht0 tht1 dtht1 s m L d D k beta tau0 tau1 b g phi footX footY dfootX dfootY hipX hipY dhipX dhipY real
...
%this script includes the potential enegry from the spinginess of the leg
%in the potential enegry calculation
...
%ideal hip joint (dont add final stance leg curvature to swing leg) and
%tht1 is a the base of the leg, so the curvature decreases along its length
%from (tht1+q1) to just tht1
alpha1 = -int(q1,s,0,s) + tht1 + q1; 
...
PEgrav = int(int((m*g*(sin(phi)*xsd0 + cos(phi)*ysd0 + sin(phi)*xsd1 + cos(phi)*ysd1)),d,-0.5,0.5),s,0,1);% +.5*(q.'*kappa*q);
PEspring = .5*(q.'*kappa*q);
G = jacobian(PEgrav,q).';
fprintf("Gravity Vector Found!\n")
KE = .5*(dq.'*M*dq);

E = KE + PEgrav + PEspring;% + .5*(q.'*kappa*q); 
...
%find lie derivs for hip motion
%the derivative themselves are too expensive to find symbolically, so find
%dhdq symbolically (this is more important for more complicated objective
%functions) and then find the derivs themselves numerically
h = [tht0 - tht0des;...
     tht1 + tht0];
dhdq = jacobian(h,q);
d2hdq2 = jacobian(jacobian(h,q)*dq,q); %this is for lffh, its (d/dq)(dhdq*dq)
...
matlabFunction(M,C,G,E,PEgrav,PEspring,KE,"file","dynMatsRigid")%,"Optimize",false)
% matlabFunction(G,"file","groupActionFootForm") %redundant
matlabFunction(Me,E2,dGamma,"file","impactMappingMatsFootFormRigid")
% matlabFunction(dxSw,dySw,xSw,ySw,"file","swingFootImpact")
matlabFunction(E,"file","getEnergyRigid") %redundant
% matlabFunction(h,dhdq,d2hdq2,"file","lieDerivs")
