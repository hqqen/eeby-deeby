...
for i=1:ind_len
    ...
    phi_H_up(:,ind_len+i)=Phi(:,ind(i)+N_a);
    X_new(i)=X(ind(i));
    X_new(ind_len+i)=X(ind(i)+N_a);
end
% phi_H_up= onlinedict(Y,phi_H_up,X_new);
clear Phi
Phi=zeros(2*Num_samples,2*ind_len);
Phi=phi_H_up;
...
af = Phi*X_new;
AF = af(1:Num_samples,1)+1i*af(Num_samples+1:end,1);
AFval = abs(AF);
polar(theta,AFval','g')
% end
