...
%         af(i,:) = den;
    ...
    gamma_ch = normrnd(1,0.2,N_a,N_s);
    gamma_ph = normrnd(0,0.1,N_a,N_s);
    for m=1:N_a
        for i=1:N_s
            u_ch(m,i) = gamma_ch(m,i)/(d(m,i)^(mu/2))*cos(alpha(m,t)+zeta(m,i)+gamma_ph(m,i));
            v_ch(m,i) = gamma_ch(m,i)/(d(m,i)^(mu/2))*sin(alpha(m,t)+zeta(m,i)+gamma_ph(m,i));
        end
    end
    for i=1:N_s
        den_ch = 0;
