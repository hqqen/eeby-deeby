...
for t=1:T-1
    if t > 1
        if any(abs(grad(:,t-1)) >= 1000)
            1 == 1;
        end
    end
    u = zeros(N_a,N_s);
    v = zeros(N_a,N_s);
    gamma_ch = normrnd(1,0.1,N_a,N_s);
    for m=1:N_a
        for i=1:2
    ...
    figure(2)
    polarplot(theta(3:end),af_db(3:end,t), 'g', 'LineWidth', 5)
    % xlabel('theta (radian)')
...
%%
% figure(2)
% polarplot(theta(3:end),af_db(3:end,1), 'g', 'LineWidth', 5)
% % xlabel('theta (radian)')
% % ylabel('radiation pattern (dB)')
% set(gca, 'LineWidth', 5, 'FontSize', 35)
% grid on
