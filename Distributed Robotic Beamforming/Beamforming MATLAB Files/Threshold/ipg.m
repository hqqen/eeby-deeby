...
%         af(i,:) = den;
    ...
    error_fdb(:,t+1) = norm(af_db(:,t)-20*log10(f/max(f)),1)/N_s;
    error_f = norm(af-f,1)/N_s;
    disp(['iter: ',num2str(t),' error_db: ',num2str(error_fdb(:,t)),' error: ',num2str(error_f)])
    if t>5000
        beta = 1;
        epsilon2 = 0.9;
    end
...
% end
[p,q]=min(error_fdb(2:end));
plot(theta,af_db(:,q), 'g', 'LineWidth', 5)
hold on
plot(theta,20*log10(f/max(f)), '--k', 'LineWidth', 5)

save('af_24.mat','af_db')
save('error_24.mat','error_fdb')