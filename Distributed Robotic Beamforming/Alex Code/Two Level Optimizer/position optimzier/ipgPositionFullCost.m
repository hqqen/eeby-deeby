function [times] = ipgPhaseMag(T,f,r,rho,tht,a0,alpha0,w,f0)
...
rMin = .1; % minimum agent spacing to avoid coupling
epsSpace = .1; %agent spacing penalty coefficient
...
for t = 1:T
    ...
    % % find interagent spacing
    % for agent1 = 1:Na
    %     for agent2 = 1:Na
    %         p(agent1,agent2) = epsSpace*log(abs(norm(r(:,agent1) - r(:,agent2)) - rMin));
    %         p(agent1,agent1) = -eps1*(x(agent1) - x0(agent1))/norm(r(agent1,:) - r0(agent1,:));
    %     end
    % end
    ...
    %         end
    %     end
    pd = getCostDerivs([x;y],[x0;y0],rMin,delta,epsSpace);
    h = getHessianFullCost(x,y,x0,y0,a,alpha,rho,tht,rMin,delta,epsSpace);

    if t > 1 && (any(isnan(pd)) || any(pd == Inf))
        1 == 1;
    end
    ...
    if t == 1
        ...
        grad(:,t) = [sum(gx,2);sum(gy,2)] + .25*sign(pd).*min(abs(pd),20);            % agent-wise gradient is summed
    end
    ...
    if ~mod(t,1)
        if ~mod(t,10)
            disp(sprintf("Average time taken over the past 10 steps: " + sum(times(t-9:t))/10 + " sec"))
        end
        figure(20202);
