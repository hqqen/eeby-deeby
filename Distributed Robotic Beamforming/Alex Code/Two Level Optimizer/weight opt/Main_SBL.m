...
% figure(2);
% scatter(rm_d(1,:),rm_d(2,:), 'LineWidth', 5);
% title("Agent Array for Des'd Beampattern")
...
% figure(3);
% scatter(rm(1,:),rm(2,:),'LineWidth', 5);
% title("Randomly Initialized Array Elements")
% grid on
...
n_w=2*N_a;
for cc=1:5
    [X,~,~,~] = MSBL(Phi,Y,lambda_SBL,Learn_Lambda,'prune_gamma',1e-4,'max_iters',500,'epsilon',1e-8,'print',0);
...
figure(41);
scatter(rm(1,abs(KK)>0),rm(2,abs(KK)>0),'b','LineWidth',4); hold on;
scatter(rm(1,abs(KK)==0),rm(2,abs(KK)==0),'r','LineWidth',4)
