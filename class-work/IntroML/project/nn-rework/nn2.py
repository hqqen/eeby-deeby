...
class acroNN(nn.Module):
    def __init__(self, nIn = 6, nHide = 8, nOut = 3):#6, 32, 3
    ...
    def act(self, state):
        ...
        m = Categorical(probs)
        #nextAction = m.sample()
        nextAction = np.random.choice(env.action_space.n, p = probs.detach().numpy().reshape(1,-1)[0])
        return nextAction, np.log(probs.detach().numpy()[0][nextAction]) #nextAction.item() - 1, m.log_prob(nextAction)
...
def reinforce(network, nEps = 1500, tMax = 1000, gamma = 1., nPrint = 100): #set tiem back to 100????
    ...
    score = []
    opt= optim.Adam(network.parameters(), lr=0.0025)
...
plt.title("Training Score Curve for Acrobot")
plt.xlabel('nEpoch #')
